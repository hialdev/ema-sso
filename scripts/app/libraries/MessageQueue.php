...
class MessageQueue extends Phalcon\Di\Injectable
{
     ...
     * @throws Exception
     */
    public function email($to, $subject, $body, $tube = null)
    {
    ...
    public function telegramAfterAdd($phone, $firstName, $lastName, $message)
    {
       ...
       /**
     * Sends Whatsapp message (WA Message)
     *
     * @param array $msisdn
     * @param string $message
     * @return bool
     * @throws Exception
     */
    public function whatsapp($msisdn, $message)
    {
        if (empty($this->queue))  $this->connect();

        $tube = $this->config->queue->tubes->whatsapp;
