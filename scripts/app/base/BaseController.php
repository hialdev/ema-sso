...
class BaseController extends Phalcon\Mvc\Controller
{
    ...
    private function setBrowserId ()
    {
        ...
        if ($_cookie = $this->cookies->get($cookieID))
            $this->browserId = $_cookie->getValue();
    ...
    {
        if ($authParams = $this->cookies->get($this->config->cookie->sso->id))
        {
            if ($auth = json_decode($authParams->getValue()))
            {
            ...
            if ($menu->menu)
            {
                foreach ($menu->menu as $submenu)
                {
                    ...
                    $child = [
                        ...
                        'desc'      => $submenu->desc,
                        'header'    => false,
                        'show'      => isset($submenu->show) ? $submenu->show : true,
                    ...
                    if ($submenu->menu)
                    {
                        foreach ($submenu->menu as $menuchild)
                        {
                            ...
                            $subchild = [
                                ...
                                'desc'      => $menuchild->desc,
                                'header'    => false,
                                'show'      => isset($menuchild->show) ? $menuchild->show : true,
                            ...
                            if (isset($menuchild->menu) && $menuchild->menu)
                            {
                                ...
                                foreach ($menuchild->menu as $menugrandchild)
                                {
                                    ...
                                    $grandchild = [
                                        ...
                                        'header'    => false,
                                        'show'      => isset($menugrandchild->show) ? $menugrandchild->show : true,
